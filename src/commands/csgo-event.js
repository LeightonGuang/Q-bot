    ...
    if (subCommand === "ongoing-events") {
      ...
      axios.get(url + "/events").then(urlResponse => {
        ...
        $("#FEATURED").find(".ongoing-event-holder").find('a').each((i, a) => {
          ...
          let ongoingEventDate = $(a).find("span.col-desc").find("span").find("span").text();
          let dateArray = ongoingEventDate.split(' ');
          let dateArrayLength = dateArray.length;
          ongoingEventDate = (`${dateArray[0]} ${dateArray[1]} - ${dateArray[dateArrayLength - 3]} ${dateArray[dateArrayLength - 2]}`);
          ...
          let ongoingEventOrganizerImgLink = $(a).find("img.logo").attr("src");

          let ongoingEventEmbed = new EmbedBuilder()
            .setAuthor({ name: ongoingEventOrganizer, iconURL: ongoingEventOrganizerImgLink })
            .setColor(0xFFFFFF)
